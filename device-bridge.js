...
const state={devices:[],loading:false,lastError:'',overlay:null,pairTimer:null,managerVersion:0,initialized:false,commandId:''};
...
function personReady(){return !(window.APPIAuth&&window.APPIAuth.needsPersonChoice&&window.APPIAuth.needsPersonChoice())}
...
function clearCommandQuery(){state.commandId='';const url=new URL(location.href);url.searchParams.delete('bridge_call');history.replaceState(history.state,'',url.pathname+url.search+url.hash)}
function rememberCommandLink(commandId){const url=new URL(location.href);url.searchParams.set('bridge_call',commandId);history.replaceState(history.state,'',url.pathname+url.search+url.hash)}
function handleServiceWorkerMessage(event){const data=event&&event.data||{},command=String(data.command_id||'');if(data.type!=='APPI_OPEN_COMMAND'||!validUuid(command))return;rememberCommandLink(command);if(authorized()&&personReady())setTimeout(()=>showCommand(command),0)}
