...
(function(){
  ...
  function css(){
    ...
    st.textContent = [
      '#view-campus{background:radial-gradient(circle at 4% 0,rgba(64,208,201,.19),transparent 28%),radial-gradient(circle at 95% 100%,rgba(117,102,220,.13),transparent 32%),#f6fbfc}',
  ...
  function mostrarCelebracion(){
    ...
    abrirModal(
      ...
      '<div class="cp-modal-foot"><p class="cp-hint">Respondé 5 preguntas de tu nivel, basadas en los temas de Campus PSA, y activá tu Impulso del día.</p><button type="button" class="cp-primary" id="cpStartQuiz">🎓 Ganar 3 más</button><button type="button" class="cp-secondary" id="cpCloseCelebrate">Terminé por hoy</button></div>'
    );
  ...
  function render(){
    ...
    host.innerHTML =
      ...
      '<button type="button" class="cp-main-btn ' + (ready ? 'ready' : '') + '" id="cpMainAction" ' + (!ready && !unlocked ? 'disabled' : '') + '>' + esc(actionLabel) + '</button><p class="cp-note">Las preguntas son originales de APPI, organizadas por los temas de formación de Campus PSA. No reemplazan sus capacitaciones oficiales.</p></div>';
