...
(function(){
  ...
  function firma(c){
    var d = digitos(c && c.telefono);
    if (d) return 'p:' + d;
    return (c && c.nombre) ? 'n:' + claveNombre(c.nombre) : 'i:' + String(c && c.id || '');
  }
  function iguales(a, b){ return !!a && !!b && firma(a) === firma(b); }
  function agendaPersonal(){
    try {
      ...
      return (lista || []).filter(function(c){ return c && c.nombre; }).map(function(c){
        return { id:String(c.id || c.tel_norm || c.telefono || c.nombre), nombre:String(c.nombre), telefono:String(c.telefono || ''), origen:'agenda' };
      });
    }catch(e){ return []; }
  }
  function panelContactos(){
    try {
      var g = window.APPIGestion && window.APPIGestion.state;
      return (g && Array.isArray(g.contacts)) ? g.contacts : [];
    }catch(e){ return []; }
  }
  function candidatos(){
    var mapa = new Map();
    function sumar(lista, origen){
      (Array.isArray(lista) ? lista : []).forEach(function(c){
        if (!c || !c.nombre) return;
        var cifra = {
          id: String(c.id || c.telefono_normalizado || c.tel_norm || c.telefono || c.nombre),
          nombre: String(c.nombre),
          telefono: String(c.telefono || ''),
          origen: origen
        };
        var k = firma(cifra) || cifra.id;
        var prev = k ? mapa.get(k) : null;
        if (!prev || (origen === 'panel' && prev.origen === 'agenda')) mapa.set(k, cifra);
      });
    }
    sumar(panelContactos(), 'panel');
    sumar(agendaPersonal(), 'agenda');
    return Array.from(mapa.values()).sort(function(a, b){ return a.nombre.localeCompare(b.nombre, 'es-AR', { sensitivity:'base' }); });
  }
  function origenDe(c){
    try {
      if (panelContactos().some(function(p){ return iguales(p, c); })) return 'panel';
      if (agendaPersonal().some(function(a){ return iguales(a, c); })) return 'agenda';
    }catch(e){}
    return 'telefono';
  }
  ...
  async function propsSoportadas(picker){
    try {
      ...
      return {
        name: Array.isArray(props) && props.indexOf('name') >= 0,
        tel: Array.isArray(props) && props.indexOf('tel') >= 0
      };
  ...
  async function pedirDelTelefono(){
    ...
    (Array.isArray(lista) ? lista : []).forEach(function(c){
      ...
      var tel = Array.isArray(c.tel) ? c.tel[0] : c.tel;
      if (!nombre) return;
      salida.push({ id:String(tel || nombre || ''), nombre:String(nombre), telefono:String(tel || '') });
    });
    return salida;
  }
