...
(function(){
  ...
  function css(){
    ...
    st.textContent = [
      '#view-margarita{min-height:100vh;background:radial-gradient(circle at 15% 4%,rgba(242,196,111,.22),transparent 27%),radial-gradient(circle at 90% 17%,rgba(225,194,146,.24),transparent 28%),linear-gradient(155deg,#f7f1e6,#f3ecdf 52%,#eee4d3)}',
  ...
  function render(){
    ...
    var petals = state.grupos.map(function(g, i){
      ...
      return '<button type="button" class="mg-petal ' + (count ? 'has-contacts' : '') + '" style="--angle:' + (i * 45) + 'deg;--counter:-' + (i * 45) + 'deg;--petal-outset:-78px;--i:' + i + '" data-mg-group="' + esc(g.id) + '"><span class="mg-petal-shape"></span><span class="mg-petal-content"><i>' + esc(g.icon) + '</i><b>' + label + '</b><small>' + count + ' persona' + (count===1?'':'s') + '</small></span></button>';
    }).join('');
    host.innerHTML =
