...
(function () {
  ...
  window.canillasSelectAlternative = function (faucetId) {
    var found = FAUCETS_DB.find(function (f) { return f.id === faucetId; });
    if (!found) return;

    currentResult = {
      faucetName: found.name,
      adapter: found.adapter_id,
      adapterName: found.adapter_name,
      pageImg: found.page_img,
  ...
  function renderResult() {
    ...
    if (currentAlternatives && currentAlternatives.length > 0) {
      altHtml = `
        <div style="margin-top: 14px; padding-top: 12px; border-top: 1px dashed rgba(11,88,120,.2);">
          <div style="font-size: 11.5px; font-weight: 800; color: #64748b; margin-bottom: 6px;">
            ¿Se parece más a alguna de estas?
          </div>
          <div style="display: flex; flex-wrap: wrap; justify-content: center;">
            ${currentAlternatives.map(function(alt) {
              return `<button type="button" class="can-alt-pill" onclick="window.canillasSelectAlternative('${alt.id}')">
                ${alt.name} (${alt.adapter_id})
              </button>`;
            }).join("")}
          </div>
        </div>
    ...
    host.innerHTML = `
      <div class="can-simple-card" style="border: 1.5px solid #0b5878; text-align: center;">
        ...
        <div style="font-size: 11px; font-weight: 900; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px;">
          MODELO RECONOCIDO: ${res.faucetName}
        </div>
        ...
        <div class="can-page-preview-box" onclick="window.canillasOpenImagePopup('${res.pageImg}', 'Ficha oficial ${res.faucetName} - ${res.adapter}')" title="Tocar para ampliar">
