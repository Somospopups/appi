/* ============================================================
   ...
   (OAuth 2.0 + PKCE, sin backend ni secret):
     ...
     1. UN TOQUE: "Conectar mi Google" → selector de cuentas →
        "Permitir" (una vez). El Client ID viene quemado en este
        archivo (CLIENT_ID_DEFAULT); para cambiar de proyecto se
        usa guardarClientId() (queda en localStorage).
     2. La app crea (si no existe) un calendario "APPI" y escribe
     ...
     3. Cada vez que cambia la base de usuarios se resincroniza
...
(function () {
  ...
  // Cliente OAuth "APPI" (Aplicación web) del proyecto My First
  ...
  // Client ID en vigor: el sobrecargado con guardarClientId() si
  // existe, si no el quemado de fábrica. (v820: ya no hay UI para
  // pegarlo — conexión de un toque.)
  function clientIdEfectivo() {
    var st = load();
    return (st && String(st.clientId || '').trim()) || CLIENT_ID_DEFAULT;
  }
