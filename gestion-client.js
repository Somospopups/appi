...
function installStyles(){
  ...
  @media(max-width:520px){.survey-actions{grid-template-columns:1fr 1fr}.survey-actions .wa{grid-column:1/-1}.survey-flow{grid-template-columns:1fr}.gestion-stats{grid-template-columns:repeat(2,1fr)}.gestion-contact-actions{grid-template-columns:1fr 1fr}.gestion-contact-actions .detail{grid-column:1/-1}.gestion-status-grid{grid-template-columns:1fr 1fr}.gestion-drawer{padding-left:12px;padding-right:12px}}
  `;document.head.appendChild(style);
}

function surveyUrl(){
  if(!state.link||!state.link.token)return '';
  const base=new URL('./encuesta.html',location.href);base.search='';base.hash='';base.searchParams.set('t',state.link.token);return base.toString();
}
function shareMessage(){
  const p=profile(),name=p&&p.nombre?` Soy ${p.nombre}.`:'';
  return `Hola 👋${name}\n\nTe invito a responder Mi Encuesta sobre agua, medio ambiente y actividad laboral. Tus respuestas me van a ayudar a conocerte mejor y se guardarán de forma segura en APPI.\n\n${surveyUrl()}`;
}
function isAdmin(){return profile()&&profile().rol==='admin'}

async function loadLink(force=false){
  if(state.link&&!force)return state.link;
  if(!authorized())throw new Error('Iniciá sesión para usar Mi Encuesta.');
  if(isAdmin())return null;
  const rows=await cloudFetch(`/rest/v1/appi_encuesta_links?select=token,activo&user_id=eq.${encodeURIComponent(userId())}&limit=1`);
  state.link=Array.isArray(rows)?rows[0]||null:null;
  if(!state.link)throw new Error('Todavía no se creó el enlace de esta cuenta.');
  return state.link;
}
...
function renderSurveyTool(){
  ...
  if(isAdmin()){
    c.innerHTML=`<div class="gestion-empty"><div class="ico">🔒</div><h3>Herramienta para distribuidores</h3><p>POPUPS administra APPI, pero no tiene número de distribuidor. Cada distribuidor verá acá su propio enlace permanente.</p></div>`;return;
  }
  if(!state.link){c.innerHTML='<div class="gestion-empty"><div class="spinner"></div><h3>Preparando tu enlace</h3><p>APPI está buscando el enlace permanente de tu cuenta.</p></div>';return}
  const url=surveyUrl();
  c.innerHTML=`
    <div class="gestion-hero"><div class="eyebrow">Enlace personal permanente</div><h2>Compartí. Recibí. Gestioná.</h2><p>La persona responde desde su celular y los datos llegan automáticamente a tu Mi Gestión.</p></div>
    <section class="gestion-card"><div class="gestion-card-head"><div><h3>Tu enlace de Mi Encuesta</h3><p>Siempre es el mismo y está asociado únicamente a tu cuenta.</p></div><span class="gestion-badge">Activo</span></div><div class="survey-link-box"><label>Enlace seguro</label><div class="survey-link-value">${esc(url)}</div></div><div class="survey-actions"><button type="button" class="wa" id="surveyShareWa">💬 Compartir por WhatsApp</button><button type="button" class="copy" id="surveyCopy">Copiar enlace</button><button type="button" class="preview" id="surveyPreview">Ver encuesta</button></div></section>
    <section class="gestion-card"><div class="gestion-card-head"><div><h3>Así funciona</h3><p>No necesitás importar mensajes ni copiar datos.</p></div></div><div class="survey-flow"><div><b>1</b><strong>Compartís</strong><small>Enviás el enlace por WhatsApp.</small></div><div><b>2</b><strong>Responden</strong><small>Completan Agua, Laboral y Referidos.</small></div><div><b>3</b><strong>Gestionás</strong><small>Aparecen automáticamente en APPI.</small></div></div></section>
    <div class="gestion-notice"><b>Privacidad:</b> cada respuesta queda ligada a tu usuario. Ningún otro distribuidor puede verla. Los referidos son opcionales y la persona debe confirmar que tiene autorización para compartirlos.</div>`;
  const share=$('surveyShareWa'),copy=$('surveyCopy'),preview=$('surveyPreview');
  if(share)share.onclick=()=>window.open(`https://wa.me/?text=${encodeURIComponent(shareMessage())}`,'_blank','noopener');
  if(copy)copy.onclick=()=>copyText(url);
  if(preview)preview.onclick=()=>window.open(url,'_blank','noopener');
}
...
async function openEncuestaTool(){
  ...
  renderSurveyTool();
  try{await loadLink();renderSurveyTool();const current=document.querySelector('.view.active')?.id;if(current==='view-home'&&typeof window.showView==='function')window.showView('view-encuesta')}catch(error){const c=$('surveyToolContent');if(c)c.innerHTML=`<div class="gestion-empty"><div class="ico">⚠️</div><h3>No pudimos cargar tu enlace</h3><p>${esc(error.message)}</p><button type="button" class="gestion-primary" id="surveyRetry" style="margin-top:12px">Reintentar</button></div>`;if($('surveyRetry'))$('surveyRetry').onclick=()=>{state.link=null;openEncuestaTool()}}
}
...
window.APPIGestion={state,open:openMiGestion,refresh:refreshManagement,loadLink,surveyUrl,flushQueue,updateBadges};
