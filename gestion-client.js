...
async function openMiGestion(){if(typeof window.showView==='function')window.showView('view-gestion');if(!authorized()){const c=$('gestionContent');if(c)c.innerHTML=emptyManagement('🔒','Iniciá sesión','El Panel de Contactos necesita una cuenta para identificar tus contactos.');return}try{state.agenda=localStorage.getItem('appi_gestion_agenda_vista_'+userId())==='personal'?'personal':'appi'}catch(e){state.agenda='appi'}if(!state.contacts.length)loadCache();renderManagement();await refreshManagement(!state.contacts.length);const current=document.querySelector('.view.active')?.id;if(current==='view-home'&&typeof window.showView==='function')window.showView('view-gestion');if(navigator.onLine)setTimeout(()=>{migrarContactosLocales().catch(()=>{})},700)}
