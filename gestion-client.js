...
const STATUSES={
  ...
  presentacion:{label:'Presentación',icon:'🎯',color:'#3ad0a4'},
...
function installShareStyles(){
  ...
  .share-btn{position:relative;width:min(100%,330px);min-height:130px;border:0;border-radius:28px;cursor:pointer;overflow:hidden;color:#fff;font:inherit;background:linear-gradient(135deg,#3c67ca 0%,#725bd6 62%,#3ad0a4 100%);box-shadow:0 16px 34px rgba(72,82,180,.3);transition:transform .18s ease,box-shadow .18s ease;-webkit-tap-highlight-color:transparent}
  ...
  .share-row .who{width:32px;height:32px;flex:none;border-radius:11px;display:grid;place-items:center;color:#fff;font-size:14px;background:linear-gradient(135deg,#0b5878,#3ad0a4)}
...
function installGenteStyles(){
  ...
  .gente-add:hover,.gente-add:focus-visible{background:#f3ecff;border-color:#3ad0a4}
  .gente-add.abierto{background:#efe6ff;border-style:solid;border-color:#3ad0a4}
  ...
  .gente-guardar{margin-top:2px;border:0;border-radius:13px;padding:13px;background:linear-gradient(135deg,#0b5878,#3ad0a4);color:#fff;font-weight:850;font-size:15px;cursor:pointer}
...
const HOY_POR_TANDA=50;
function renderTodayView(){const actions=actionableContacts(),hoyVisibles=actions.slice(0,HOY_POR_TANDA),newCount=countStatus('nuevo'),today=state.contacts.filter(isTodayContact).length,overdue=state.contacts.filter(isOverdueContact).length,presentations=countStatus('presentacion'),withoutDate=state.contacts.filter(c=>c.estado==='seguimiento'&&!c.proximo_contacto).length;return `<div class="gestion-stats"><div class="gestion-stat"><span>✨</span><b>${newCount}</b><small>Nuevos</small></div><div class="gestion-stat"><span>☀️</span><b>${today}</b><small>Para hoy</small></div><div class="gestion-stat"><span>⚠️</span><b>${overdue}</b><small>Vencidos</small></div><div class="gestion-stat"><span>🎯</span><b>${presentations}</b><small>Presentaciones</small></div></div><div class="gestion-section-title"><h3>Prioridad de hoy</h3><small>${actions.length} acción${actions.length===1?'':'es'}</small></div><div class="gestion-list">${actions.length?hoyVisibles.map(contactCard).join(''):(actions.length>0?'':emptyManagement('🎉','Todo al día','No tenés contactos pendientes para hoy.'))}${actions.length>HOY_POR_TANDA?`<button type="button" class="gestion-secondary" id="gestionVerTodos" style="width:100%;margin-top:10px">Ver las ${actions.length-HOY_POR_TANDA} restantes en “Todos”</button>`:''}</div>${withoutDate?`<div class="gestion-notice" style="margin-top:11px"><b>📅 ${withoutDate} seguimiento${withoutDate===1?'':'s'} sin fecha.</b> Abrí cada contacto y programá el próximo paso para que no quede olvidado.</div>`:''}`}
...
// Render limitado por tanda: con listas grandes (miles de contactos) pintar
// todas a la vez armaba un DOM de varios MB que en iOS dejaba la pantalla
// blanca (Safari mata el webview por memoria). 100 por tanda y botón para
// seguir; búsqueda/filtros siempre corrigen sobre el total real.
const LISTA_POR_TANDA=100;
function resetTandaLista(){state.mostrarHasta=LISTA_POR_TANDA}
function renderAllView(){const list=filteredContacts();if(!Number.isFinite(state.mostrarHasta))resetTandaLista();const visibles=list.slice(0,state.mostrarHasta),quedan=list.length-visibles.length;return `<div class="gestion-toolbar"><div class="gestion-search"><input id="gestionSearch" type="search" autocomplete="off" placeholder="Buscar por nombre, teléfono, zona…" value="${esc(state.search)}"><span>⌕</span></div><div class="gestion-filters">${[{id:'todos',label:'Todos'},...Object.entries(STATUSES).map(([id,v])=>({id,label:v.label}))].map(item=>`<button type="button" class="gestion-filter ${state.filter===item.id?'active':''}" data-gestion-filter="${item.id}">${esc(item.label)}${item.id==='todos'?` · ${state.contacts.length}`:` · ${countStatus(item.id)}`}</button>`).join('')}</div></div><div class="gestion-refresh-row"><span>${state.lastLoaded?`Actualizado ${esc(formatDate(state.lastLoaded,true))}`:'Sin actualizar'}${loadQueue().length?` · ${loadQueue().length} pendiente${loadQueue().length===1?'':'s'}`:''}</span><div><button type="button" id="gestionExport">Exportar CSV</button><button type="button" id="gestionRefresh">Actualizar</button></div></div><div class="gestion-list">${visibles.length?visibles.map(contactCard).join(''):emptyManagement('📭',state.contacts.length?'No hay coincidencias':'Todavía no hay contactos',state.contacts.length?'Probá otro nombre o estado.':'Compartí una invitación desde Mi Encuesta.')}${quedan?`<button type="button" class="gestion-secondary" id="gestionMore" style="width:100%;margin-top:10px">Mostrar ${Math.min(LISTA_POR_TANDA,quedan)} más de ${quedan}</button>`:''}</div>`}
...
function renderManagement(){const c=$('gestionContent');if(!c)return;try{renderManagementInterno(c)}catch(err){state.lastError=String(err&&err.message||err);try{c.innerHTML=emptyManagement('⚠️','No pudimos dibujar el Panel',state.lastError+' — tus datos siguen a salvo.','<button type="button" class="gestion-primary" id="gestionRetryRender" style="margin-top:12px">Reintentar</button>');const r=$('gestionRetryRender');if(r)r.onclick=()=>{state.lastError='';renderManagement()}}catch(e){}}}
function renderManagementInterno(c){const abierto=$('gestionDetailOverlay'),form=$('genteForm');if(state.enviando||(abierto&&!abierto.hidden)||(form&&!form.hidden)){state.reRender=true;return}// La Agenda Personal tiene su propia caché y no debe ser reemplazada
...
function bindManagement(){/* v358: la barra de tarjetas de crédito se quitó del panel (sigue en Usuarios); el switch de agendas se bindea siempre */try{if(window.APPIAgendaPersonal&&window.APPIAgendaPersonal.bind)window.APPIAgendaPersonal.bind()}catch(e){}document.querySelectorAll('[data-gestion-view]').forEach(button=>button.onclick=()=>setManagementView(button.dataset.gestionView));if($('gestionSearch'))$('gestionSearch').oninput=e=>{state.search=e.target.value;resetTandaLista();const pos=e.target.selectionStart;renderManagement();const input=$('gestionSearch');if(input){input.focus();try{input.setSelectionRange(pos,pos)}catch(err){}}};document.querySelectorAll('[data-gestion-filter]').forEach(button=>button.onclick=()=>{state.filter=button.dataset.gestionFilter;resetTandaLista();renderManagement()});document.querySelectorAll('[data-funnel-status]').forEach(button=>button.onclick=()=>{state.filter=button.dataset.funnelStatus;setManagementView('todos')});document.querySelectorAll('[data-open-contact]').forEach(button=>button.onclick=()=>openContactDetail(button.dataset.openContact));if($('gestionRefresh'))$('gestionRefresh').onclick=()=>refreshManagement(true);if($('gestionVerTodos'))$('gestionVerTodos').onclick=()=>setManagementView('todos');if($('gestionMore'))$('gestionMore').onclick=()=>{state.mostrarHasta=(Number(state.mostrarHasta)||LISTA_POR_TANDA)+LISTA_POR_TANDA;renderManagement()};if($('gestionExport'))$('gestionExport').onclick=exportCsv;if($('gestionGoSurvey'))$('gestionGoSurvey').onclick=openEncuestaTool;if($('surveyShareBtn'))$('surveyShareBtn').onclick=startShareFlow;if($('genteNuevo'))$('genteNuevo').onclick=nuevaPersonaManual;
...
async function openMiGestion(){
  ...
  // v831: si la agenda personal falla (red, datos raros), el Panel ya está
  // pintado arriba; el fallo se registra y no deja la pantalla en blanco.
  try{await agendaSync}catch(err){console.warn('Sync agenda personal al abrir:',err)}
  try{await agendaCuenta}catch(err){console.warn('Cuenta agenda personal al abrir:',err)}
