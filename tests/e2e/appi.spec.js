...
test('arranca, navega e importa Garantías una sola vez', async ({ page }) => {
  ...
  expect(sidebarButtons.map(text=>text.replace(/^[^\p{L}]+/u,'').trim())).toEqual([
    'Home','Las 7 P','Presupuesto','Rueda de la Vida','Rueda del Negocio','Mi Equipo','Histórico','Usuarios','Panel de Contactos 0','Mi stock','Los 8 Pasos','Escalera de Sueños','Coach de Demo','Comparativas','Lista de precios','Avisos por Telegram','Reuniones','Banco de Imágenes','Simulador','Grabadora','Notas Keep'
  ]);
  ...
  await page.evaluate(() => {
    ...
    // La apertura real va por APPITel -> APPIWhatsApp (whatsapp-app.js ya
    // guardó su propio window.open al cargar, así que pisar el global no
    // alcanza): capturamos el abrir de APPIWhatsApp.
    window.APPIWhatsApp = { abrir: (...args) => { window.__appiLastOpen = args; return null; } };
