...
(function () {
  ...
  function estilo() {
    var css = '' +
      ...
      '.lp-qty{display:flex;align-items:center;justify-content:flex-end;gap:5px;flex-shrink:0;width:82px;min-width:82px}' +
      '.lp-qty button{width:32px;height:32px;flex:none;border:0;border-radius:10px;background:#0b5878;color:#fff;font:inherit;font-size:18px;font-weight:900;line-height:1;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:transform .1s ease}' +
      '.lp-qty button:active{transform:scale(0.92)}' +
      '.lp-qty button.ghost{background:rgba(11,88,120,.12);color:#0b5878}' +
      '.lp-qty i{width:18px;text-align:center;font-style:normal;font-size:13px;font-weight:900;color:#2a2a32;flex:none}' +
      '.lp-empty{padding:28px 8px;text-align:center;font-size:13px;font-weight:750;color:#686977}' +
      '@keyframes lpFabAppear{0%{opacity:0;transform:translateX(-50%) translateY(110px) scale(0.92)}55%{opacity:1}100%{opacity:1;transform:translateX(-50%) translateY(0) scale(1)}}' +
      ...
      '.lp-fab.anim-enter{animation:lpFabAppear 1.05s cubic-bezier(.12,.95,.22,1) both}' +
      ...
      'body.dark .lp-fab-badge{background:#fff;color:#074a66}' +
      '@media (min-width:1024px){.lp-fab{bottom:24px;left:50%;right:auto;transform:translateX(-50%)}.lp-wrap{padding-bottom:88px}}' +
      ...
      '.lp-gan{margin:0 0 14px;border:1px solid rgba(11,88,120,.22);border-radius:16px;background:linear-gradient(135deg,rgba(11,88,120,.07),rgba(58,208,164,.10));padding:12px 14px}' +
      ...
      '.lp-gan-num{font-size:24px;font-weight:950;color:#0b5878;margin:2px 0 8px}' +
      '.lp-gan-num i{font-style:normal;font-size:11.5px;font-weight:850;color:#23826b;margin-left:8px}' +
      '.lp-gan-cards{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 10px}' +
      '.lp-gan-card{background:#ffffff;border-radius:12px;padding:9px 10px;border:1px solid rgba(0,0,0,.08);box-shadow:0 2px 6px rgba(0,0,0,.04);display:flex;flex-direction:column}' +
      '.lp-gan-card-tit{font-size:10.5px;font-weight:800;color:#52525b;display:flex;align-items:center;gap:4px;margin-bottom:3px}' +
      ...
      'body.dark .lp-gan-card-tit{color:#a1a1aa}' +
      'body.dark .lp-gan-card-sub{color:#94a3b8}' +
      'body.dark .lp-gan-card-val{color:#38bdf8}' +
      'body.dark .lp-gan-card.saldo-card .lp-gan-card-val{color:#34d399}' +
      ...
      '.lp-actions .lp-clear{width:44px;height:44px;flex:none;border-radius:50%;border:1px solid #fda4af;background:#ffe4e6;color:#e11d48;font-size:18px;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 8px rgba(225,29,72,.18);transition:transform .12s ease,background .15s ease}' +
      '.lp-actions .lp-clear:active{transform:scale(0.92);background:#fecdd3}' +
      ...
      'body.dark .lp-actions .lp-clear{background:#3f1722;color:#fb7185;border-color:#881337}' +
      ...
      '@media (min-width:1024px){' +
        '.lp-fab{bottom:24px;left:50%;right:auto;transform:translateX(-50%)}' +
  ...
  function pintarFab() {
    ...
    if (eraOculto) {
      ...
      if (r.n > _prevFabCount) sonarGotita();
    }
    _prevFabCount = r.n;
  ...
  function actualizarFilaQty(sku) {
    var q = qty(sku);
    var row = document.querySelector('#lpList .lp-item[data-sku="' + CSS.escape(sku) + '"]');
    if (!row) return false;
    var qtyDiv = row.querySelector('.lp-qty');
    if (!qtyDiv) return false;
    if (q > 0) {
      qtyDiv.innerHTML = '<button type="button" class="ghost" data-act="menos" aria-label="Quitar">−</button><i>' + q + '</i><button type="button" data-act="mas" aria-label="Agregar">+</button>';
    } else {
      qtyDiv.innerHTML = '<button type="button" data-act="mas" aria-label="Agregar">+</button>';
    }
    return true;
  }
  ...
  function bind() {
    ...
    if (sheet && !sheet._lpBound) {
      ...
      sheet.onclick = function (e) {
        ...
        var sku = row.getAttribute('data-sku');
        var n = qty(sku);
        var isMenos = btn.getAttribute('data-act') === 'menos';
        if (isMenos) sonarQuitar();
        setQty(sku, isMenos ? n - 1 : n + 1);
        pintarTodo();
      };
    }
  }
