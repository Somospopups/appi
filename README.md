...
- Versión: **v843 · Segura** · 📸🔍 **Lista de Precios: 312/312 fotos del Portal PCD + popup de foto agrandada + bug de bump corregido (CI verde)**. (1) Corrección del bump que dejaba la CI roja desde la v835: `package.json` traía la versión sin el sufijo `.0.0` (el test `pwa-cache` exige el formato `N.0.0`) en v835, v836, v840, v841 y v842, y esta línea del README se había quedado a medio actualizar desde la v835 (referenciaba la cache `appi-v833-pcd-fotos`); los 6 lugares vuelven a estar alineados. (2) La Edge Function `actualizar-precios` ahora reconstruye también por nombre exacto: un producto que perdió su SKU o su `foto` en el bucket los recupera desde `psa-catalogo.json` del repo, de modo que el botón "Actualizar precios" deja el catálogo publicado con las 312 fotos del Portal PCD (comprasonline.psa.com.ar) y sus SKUs. (3) Estado consolidado: 312/312 productos con foto oficial del PCD (v834), fotos en Plan Canje + popup de foto agrandada (v835), Packs PSA y fichas en el PDF de presupuesto (v836), descripciones desde el catálogo local (v838-v840), "qué trata y por qué sacarlo" en el PDF (v841) y calendario Tu Mes simplificado (v842). · Cache `appi-v843-fotos-pcd`.
