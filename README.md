...
- Versión: **v828 · Segura** · 🟢 **CI verde y dos arreglos**: (1) **CI vuelve a verde** — el guard del service worker impide que un SW viejo bloquee la app (causa de la CI roja desde v826): 446 tests e2e en verde (144 + 200 + 102 en tres shards). (2) **El mazo vuelve a abrirse**: la tarjeta de presupuesto se abre al tocarla (el click se re-targeteaba sobre el `setPointerCapture` interno; ahora clickea el contenido que lo recibe). (3) **El login ya no corta tu flujo**: el sync de datos al entrar corre en segundo plano (sin bloquear la app) y al terminar NO te tira a Home si ya navegaste a otra vista (p. ej. acabaste de abrir Gestión/Encuesta) — era lo que hacía desaparecer botones.
- Versión anterior: **v827 · Segura** · 🔒 **Tu ganancia en el presupuesto — privada, jamás para el cliente**: al abrir el Presupuesto (Lista de precios → Presupuesto) aparece ARRIBA de las líneas una tarjeta **"🔒 SOLO PARA VOS"** con la ganancia total del presupuesto, el margen % y el desglose por ítem, calculada con la columna de costo de TU cuenta real (condición tributaria × categoría: RI/Mono → ri, No categorizado → nc; Líder/Coordinador/Calificado → DC-CE-LE, Junior → j) y las listas oficiales de PSA (psa-ganancias.json, generado solo desde los dos PDFs oficiales — vigencia 9-SEP-2026 — con tools/build-ganancias-psa.py); ítem sin lista → estimado al 30% (tu regla) y marcado; la línea **Plan canje** usa el costo de la lista canje. Tu perfil se **sincroniza solo** con tus credenciales PSA (categoría, tributaria, teléfono, saldo en cuenta corriente y última devolución de saldos — TTL 24 h, 0 configuración; cache `appi-v828-ci-verde`): la tarjeta muestra además tu **saldo PSA** con el recordatorio de pedir la devolución del 1° al 5 (acredita el 12) y el botón **📲 Enviarme a WhatsApp** te la manda a TU número de PSA. El PDF del cliente no cambia nunca. Arranca la acción `perfil` de consulta-serial (CI la despliega sola).
