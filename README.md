...
- Versión: **v827 · Segura** · 🔒 **Tu ganancia en el presupuesto — privada, jamás para el cliente**: al abrir el Presupuesto (Lista de precios → Presupuesto) aparece ARRIBA de las líneas una tarjeta **"🔒 SOLO PARA VOS"** con la ganancia total del presupuesto, el margen % y el desglose por ítem, calculada con la columna de costo de TU cuenta real (condición tributaria × categoría: RI/Mono → ri, No categorizado → nc; Líder/Coordinador/Calificado → DC-CE-LE, Junior → j) y las listas oficiales de PSA (psa-ganancias.json, generado solo desde los dos PDFs oficiales — vigencia 9-SEP-2026 — con tools/build-ganancias-psa.py); ítem sin lista → estimado al 30% (tu regla) y marcado; la línea **Plan canje** usa el costo de la lista canje. Tu perfil se **sincroniza solo** con tus credenciales PSA (categoría, tributaria, teléfono, saldo en cuenta corriente y última devolución de saldos — TTL 24 h, 0 configuración; cache `appi-v827-ganancia-presu`): la tarjeta muestra además tu **saldo PSA** con el recordatorio de pedir la devolución del 1° al 5 (acredita el 12) y el botón **📲 Enviarme a WhatsApp** te la manda a TU número de PSA. El PDF del cliente no cambia nunca. Arranca la acción `perfil` de consulta-serial (CI la despliega sola). · 🔧 **Se corrige la alineación de versiones que v826 dejó rota** (CI en rojo desde entonces): cacheName del paquete, README y App Shell precargando reporte-bonos.js.
- Versión anterior: **v826 · Segura** · 📊 **TOTAL PB del banner con el dato real**: el KPI PERSONAS/ACTIVOS/TOTAL PB de Mi negocio tomaba el TOTAL PB de la suma del archivo de equipo (número que no matcheaba con el tablero de PSA); ahora toma el renglón **"Lider"** del Reporte de Bonos (el acumulado real del mes) y se actualiza solo cuando llega el reporte; si el reporte no está cargado, respalda con la suma del equipo. · 🗑️ **Se retira la opción Calendario de Google** (a pedido del usuario: mientras menos pasos para la gente, mejor): desaparecen la sección y el módulo completo (google-calendar.js, sus 4 tests, precache y shard de CI); los 5 recordatorios locales del teléfono quedan intactos; el proyecto Google Cloud queda sin tocar por si se vuelve a armar. · 🤖 **Cero pasos manuales: el aviso de planilla vieja se autodispara**: si la base de Usuarios se cargó con una app anterior a la v813 (objetos sin la columna «Dip reasignado»), antes aparecía un aviso pidiendo "Recargar planilla" a mano; ahora la app **migrada los campos sola** (los valores reales llegan solos con la sync automática de MI PSA que corre al abrir la app) y el aviso no se muestra nunca más. · 🎨 **Barra del navegador a color de la app**: la barra de estado dejaba de ser negra al terminar de cargar y pasaba a un celeste claro que no matcheaba nada; ahora usa la crema de la app (`#f3eee3`) en modo día y el oscuro de la app (`#16161e`) en modo noche (el negro de carga `#06172d` se mantiene). · 🔐 **Conexión Google resuelta de una**: el cliente OAuth tenía un Client Secret autogenerado por la consola (los clientes "Aplicación web" son confidenciales por defecto) y Google lo exigió en el intercambio de token ("client_secret is missing"); ahora viene quemado en el código junto al Client ID, así que el toque único funciona completo. · 🔴 **Error de conexión Google visible**: si Google rechaza la conexión (el más común: tu cuenta no está en "Usuarios de prueba" del proyecto, que está en modo En prueba), la sección ahora muestra el error exacto con botón "Ocultar" (antes quedaba en silencio). · 📅 **Calendario de Google (un toque)**: en Mi Perfil → Notificaciones, sección "Calendario de Google". Un toque: "Conectar mi Google" → selector de cuentas de Google → "Permitir" (el Client ID viene quemado en el código, no se pega nada) (OAuth 2.0 + PKCE, sin backend ni secret). La app crea el calendario **APPI** y programa con alarmas los **cumpleaños** y las **garantías por vencer** de los próximos 60 días; los avisos los manda el propio Google Calendar (celu, compu y reloj). Cada vez que cambia la base de usuarios se resincroniza solo: borra lo que ya no aplica, crea lo nuevo y **adopta** los eventos existentes para no duplicar. Botones Sincronizar ahora / Desconectar. · 🔔 **Recordatorios del teléfono**: APPI programa avisos locales diarios (Android e iPhone, sin "conectar" nada) — 🎂 cumpleaños de hoy (08:00), ☀️ acciones del día (09:00), 📅 garantías por vencer (09:30), ↻ reasignados para recontactar (10:00) y 🌙 cierre del día (18:00). Se arman solos con los datos locales (salen aunque la app esté cerrada, vía showTrigger; si el navegador no lo soporta, salen al abrir la app y se pasó la hora, una vez por día). Al tocar el aviso, abre la vista que corresponde (Usuarios / Home). Se configuran en **Mi Perfil → Notificaciones y recordatorios** (horas, switches y botón Probar). · ↻ **Reasignados 100% automáticos**: la sync automática de MI PSA (la que se sola al abrir la app) ahora trae la columna **«Dip reasignado»** (ex distribuidor), **E Mail** y **Cumpleaños** de la planilla de Garantías — antes las tiraba afuera y pisaba la lista, que era el motivo del "0 reasignados". La función `consulta-serial` (action:'report') pasa los campos nuevos (`dr`, `e`, `cn`) y la app los guarda y muestra sin que haya que recargar nada: badge con la cantidad, pildora ↻, "Reasignado de: X" y mensaje de recontacto al tocar 💬. · ⚠️ **Aviso de planilla vieja en Reasignados**: si la planilla de Usuarios se cargó antes de la v813, sus datos no traen la columna «Dip reasignado» y el filtro marca 0 sin motivo aparente; ahora la app muestra un **aviso ámbar arriba del listado** ("Tu planilla se cargó antes de la v813…") con botón **🔄 Recargar planilla** que abre el selector de archivo con un toque. Si la planilla ya trae la columna, el aviso no aparece. · v815: ↻ Botón **Reasignados** de Usuarios: la cantidad pasó a un **badge azul en la esquina** (el texto "Reasignados ( N ) " se partía en varias líneas) y la detección de la columna de la planilla aceptó más variantes del encabezado (DIP reasig, DIP reasignado, ex DIP, DIP anterior/antiguo/viejo). · v814:  · ↻ El botón de filtro **Reasignados** del listado de Usuarios ahora está **siempre visible** (antes solo aparecía cuando había al menos uno): filtra la lista a los usuarios reasignados por la empresa y muestra su cantidad. · v813:  · ↻ **Usuarios reasignados**: la planilla trae una columna "DIP reasignado" con el nombre del ex distribuidor cuando la empresa reasigna un usuario; la app ahora la lee, los **marca con una pildora ↻** en el listado (al abrirlo, "Reasignado de: X"), y ofrece el **filtro "solo reasignados"**. Al tocarles **💬 WhatsApp**, el **primer mensaje** que se ofrece es el de **recontacto** (con vista previa y botón para mandarlo): la empresa tiene un sistema de garantías, el ex distribuidor ya no está en el sistema y queda claro quién lo atiende ahora (el nombre sale del campo "¿Cómo te gustaría que te llamemos?" de Mi Perfil; si está vacío, el del titular del Reporte de Bonos). Después del primer mensaje, las plantillas de siempre. · 💰 **Reporte de Bonos ahora es un BOTÓN ALARGADO + POPUP** (reemplaza al ojito de v811): en la parte superior de Mi negocio hay un botón azul alargado "💰 Reporte de Bonos · <período>"; al presionarlo se abre un popup (hoja inferior) con todos los datos (acumulación, detalle de bonos, total, aviso y fecha); se cierra con ✕, tocando afuera, con Esc o al salir de la vista — los datos nunca quedan a texto abierto. · Se mantiene: actualización sola al entrar a la app y al abrir Mi negocio (tope 30 min), cache offline, botón ↻, estados amarillos alineados a la derecha y el banner PERSONAS/ACTIVOS/TOTAL PB entre el botón y la grilla. · 👁 **Privacidad en el Reporte de Bonos**: los datos nacen TAPADOS (blur); el ojito 👁 del encabezado los muestra y 🙈 los vuelve a tapar; al salir o volver a entrar a Mi negocio se retapan solos. · 📏 Los estados amarillos ("faltan…") del detalle de bonos quedaron alineados a la derecha, como el resto de los renglones. · 📍 El banner de PERSONAS / ACTIVOS / TOTAL PB se movió del home (debajo del calendario) a Mi negocio, entre el Reporte de Bonos y la grilla de botones, con espaciado acorde. · 💰 El Reporte de Bonos se movió de Mi Equipo a MI NEGOCIO (como quedó pedido): sigue SIEMPRE VISIBLE en la parte superior de la pantalla, con la grilla de botones de Mi negocio debajo, y mantiene todo lo de v809: acumulación del mes, detalle de bonos (calificó / faltan / no calificó), total y aviso; se actualiza sola al entrar a la app y al abrir Mi negocio (tope 30 min) (consulta-serial action:"bonos" → dip idx=169, ISO-8859-1), queda cacheada y sin internet muestra la última copia con aviso y botón ↻. · 📅 El calendario de Tu jornada ahora es visible en el Home, justo debajo del apartado Tu jornada (antes era un modal que se abría al tocar la card; la card ahora baja a verlo). Cambiar tareas actualiza la línea de tiempo al instante. · 🗑️ Se quitó la Ruleta PSA a pedido del usuario: desaparecen el módulo (ruleta-psa.js), la demo y sus tests; Mi negocio queda sin la tarjeta GIRAR (el GPS del mes ya no se mostraba, sigue sin mostrarse). Todo lo demás del mazo v806 sigue igual. · 🃏 Mazo unificado (modelo reempadronamiento): todas las tarjetas muestran a la gente y se colorean verde (ya la hice) / rojo (no la hice), **sin botones abajo** en ninguna. Un color por tarjeta: azul (impulso, sin píldoras), violeta (Usuarios + Plan Canje unificado), dorado (Tu jornada), turquesa (promo), esmeralda (ganaste), naranja (aviso WhatsApp), fucsia (cumpleaños), verde pino (reempadronar), marino (Mi Equipo), grafito (Panel) y ladrillo (Rinnova). "Hoy te conviene" se quitó del mazo.
