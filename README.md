...
- Versión: **v835 · Segura** · 🔄 **Fotos en Plan Canje con distintivo + 🔍 Popup modal ampliado al tocar la foto de cualquier producto**. Corrección de la v832: las miniaturas de la Lista de Precios ahora son **las mismas fotos que ve el distribuidor al comprar en el Portal PCD de compras** (PNG 200×200 servidos por el CDN público `assets.psa.com.ar/uploads/normal/ipsaem/sol/tecnica/articulo/<SKU>.png`), no las de la tienda pública `tienda.psa.com.ar`. Cambios: (1) `catalogo-img/` pasó de 63 `.jpg` de tienda a **63 `.png` del PCD** (63/63 productos con foto, 4,7 MB) y el catálogo apunta a `.png`; (2) el sync nocturno `scripts/actualizar-precios-psa.py` vuelve a bajar las fotos **desde el PCD** (`bajar_foto(sku)` sin re-escalar: ya son 200×200) en cada corrida, en lugar de la `small_image` de la tienda; (3) la Edge Function `actualizar-precios` ahora **conserva el campo `foto` en el merge** del catálogo y lo **reconstruye desde `psa-catalogo.json` del repo** si se pierde (self-heal), para que el botón "Actualizar precios" no le quite las fotos a la lista; (4) `build-catalogo-psa.mjs` recupera la foto desde el archivo (`.png` primero, `.jpg` como legado) si el catálogo viejo no la llevaba; (5) e2e verifica que la miniatura CARGA de verdad (`naturalWidth > 0`) y que un producto sin foto no muestra imagen ni se rompe. La UI es la de la v832 (miniatura 44px con `object-fit:contain`, solo filas con foto). · Cache `appi-v833-pcd-fotos`.
