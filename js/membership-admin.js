...
(function(){
  ...
  function showPaymentModal(userId,userName){
    return new Promise(resolve=>{
      let cerrado=false;
      const fin=v=>{ if(cerrado)return; cerrado=true; resolve(v); };
      const modal=document.createElement('div');
      modal.className='modal-overlay membership-modal-overlay';
      modal.innerHTML=`
      ...
      modal.querySelector('.modal-close').onclick=()=>{closeModal(modal);fin(null)};
      modal.querySelector('[data-membership-cancel]').onclick=()=>{closeModal(modal);fin(null)};
