...
function categoryPB(period,category){return period.people.filter(p=>p.cat===category).reduce((sum,p)=>sum+num(p.totalPB),0)}
...
function renderAnnualSummary(year){const {periods,byMonth,rows,values}=annualMatrix(year),body=rows.map(row=>{if(row.section)return `<tr class="hist-annual-section"><th colspan="14">${esc(row.section)}</th></tr>`;const vals=Array.from({length:12},(_,m)=>values.get(`${row.id}:${m}`)||0),total=vals.reduce((a,b)=>a+b,0);return `<tr><th>${esc(row.label)}</th>${vals.map((v,m)=>{const period=byMonth.get(m);return `<td>${period?`<button data-hist-annual="${row.id}" data-period-id="${period.id}">${row.unit==='pb'?fmt(v):Math.round(v)}</button>`:'—'}</td>`}).join('')}<td><button data-hist-annual="${row.id}" data-period-id="all" data-year="${year}">${row.unit==='pb'?fmt(total):Math.round(total)}</button></td></tr>`}).join('');
  return `<div class="hist-card hist-annual-summary"><div class="hist-card-head"><div><h3>Resumen anual ${year}</h3><p>Las categorías suman organizaciones completas. Tocá cualquier número para ver su origen.</p></div><div class="hist-annual-head-actions"><span class="hist-badge">${periods.length}/12 meses</span><button type="button" class="hist-share-pdf" data-hist-share-pdf="${year}"><span>PDF</span> Compartir</button></div></div><div class="hist-annual-table-wrap"><table><thead><tr><th>Indicador</th>${SHORT_H.map(m=>`<th>${m}</th>`).join('')}<th>Total</th></tr></thead><tbody>${body}</tbody></table></div></div>`}
...
async function shareFullHistoricalReport(year){
  ...
  const card=(x,y,w,h,label,value,tone=C.blue,sub='')=>{roundRect(x,y,w,h,C.white,C.line,9);color('fill',tone);pdf.roundedRect(x+10,y+10,5,h-20,2,2,'F');setFont(7,'bold',C.muted);pdf.text(clean(label).toUpperCase(),x+24,y+18);setFont(17,'bold',C.ink);pdf.text(clean(value),x+24,y+39);if(sub){setFont(6.7,'normal',C.muted);pdf.text(clip(sub,w-34,6.7),x+24,y+h-8)}};
  ...
  // Detalle por mes
  periods.forEach(period=>{const prev=previousPeriodFor(period),summary=period.summary,cats=Object.keys(HIST_CAT_RANK).filter(cat=>period.people.some(p=>p.cat===cat)),sellers=[...period.people].sort((a,b)=>b.pnAct-a.pnAct).slice(0,10),orgs=[...period.people].sort((a,b)=>b.totalPB-a.totalPB).slice(0,10),passes=['D','DC','CE','L','LE'].flatMap(cat=>passList(period,prev,cat).map(p=>[p.nombre,`${p.codigo} · a ${cat}`])),bonus=bonusQualifications(period).flatMap(group=>group.incomes.length?[[group.sponsor.nombre,`Bonus ${group.incomes.length===1?1:2} · ${group.incomes.length} ingreso${group.incomes.length===1?'':'s'}`]]:[]);addHeader(`Detalle mensual · ${period.label}`,`${summary.people} personas · ${summary.incomeCount} ingresos`);const cardGap=8,cardW=(W-M*2-cardGap*4)/5;card(M,81,cardW,53,'PB equipo',`${fmt(summary.pbPersonal)} PB`,C.blue);card(M+(cardW+cardGap),81,cardW,53,'Actividad',`${summary.activePct}%`,C.green,`${summary.active}/${summary.people}`);card(M+(cardW+cardGap)*2,81,cardW,53,'Ingresos',String(summary.incomeCount),C.purple);card(M+(cardW+cardGap)*3,81,cardW,53,'Pendientes',fmt(summary.pending),C.amber);card(M+(cardW+cardGap)*4,81,cardW,53,'Vencidas',`${summary.expiredPct}%`,C.red,`${fmt(summary.expired)} de ${fmt(summary.presented)}`);const topY=145,panelGap=9,panelW=(W-M*2-panelGap*2)/3;tablePanel(M,topY,panelW,196,'PB organizacional por categoría',[{label:'Categoría',ratio:1.5},{label:'Personas',ratio:.7,align:'right'},{label:'PB org.',ratio:1,align:'right'}],cats.map(cat=>[cat,String(period.people.filter(p=>p.cat===cat).length),fmt(categoryPB(period,cat))]),'Sin categorías');tablePanel(M+panelW+panelGap,topY,panelW,196,'Top 10 vendedores',[{label:'Persona',ratio:1.9},{label:'Cat.',ratio:.55},{label:'PB',ratio:.7,align:'right'}],sellers.map((p,i)=>[`${i+1}. ${p.nombre}`,p.cat,fmt(p.pnAct)]),'Sin vendedores');tablePanel(M+(panelW+panelGap)*2,topY,panelW,196,'Top 10 organizaciones',[{label:'Persona',ratio:1.8},{label:'Cat.',ratio:.55},{label:'PB total',ratio:.8,align:'right'}],orgs.map((p,i)=>[`${i+1}. ${p.nombre}`,p.cat,fmt(p.totalPB)]),'Sin organizaciones');const bottomY=351,bottomW=(W-M*2-panelGap)/2;tablePanel(M,bottomY,bottomW,174,'Ingresos del mes',[{label:'Persona / DIP',ratio:1.65},{label:'Categoría',ratio:.55},{label:'Patrocinante',ratio:1.25}],(period.incomes||[]).slice(0,8).map(item=>[`${item.nombre} · ${item.dip}`,item.cat||'-',item.patrocinanteNombre||item.patrocinanteDip||'-']),'No hubo ingresos');tablePanel(M+bottomW+panelGap,bottomY,bottomW,174,'Pases y Bonus',[{label:'Persona',ratio:1.7},{label:'Detalle',ratio:1.35}],passes.concat(bonus).slice(0,8),'No hubo pases ni Bonus');if((period.incomes||[]).length>8){for(let start=8;start<period.incomes.length;start+=22){addHeader(`Ingresos · ${period.label}`,`Detalle ${start+1} a ${Math.min(start+22,period.incomes.length)} de ${period.incomes.length}`);tablePanel(M,85,W-M*2,440,'Detalle completo de Ingresos',[{label:'Nombre',ratio:1.5},{label:'DIP',ratio:.8},{label:'Cat.',ratio:.4},{label:'Patrocinante',ratio:1.5},{label:'DIP patroc.',ratio:.8},{label:'Compra',ratio:.6}],period.incomes.slice(start,start+22).map(item=>[item.nombre,item.dip,item.cat||'-',item.patrocinanteNombre||'-',item.patrocinanteDip||'-',item.compraPosterior?'Sí':'No']))}}});
