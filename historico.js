...
function categoryPB(period,category){const peopleTotal=period.people.filter(p=>p.cat===category).reduce((sum,p)=>sum+num(p.totalPB),0),includeTitular=titularCategory(period)===category&&!titularInPeople(period);return peopleTotal+(includeTitular?num(period.summary&&period.summary.pbPersonal):0)}
...
function annualRows(year){const yearPeriods=H.periods.filter(p=>p.year===year),present=new Set(yearPeriods.flatMap(p=>[...p.people.map(x=>x.cat),titularCategory(p)]).filter(Boolean)),categoryRows=Object.keys(HIST_CAT_RANK).filter(cat=>present.has(cat)).map(cat=>({id:`orgcat:${cat}`,label:HIST_CAT_LABEL[cat]||`Org. ${cat}`,unit:'pb',category:cat}));return [
...
function renderDashboard(c){
  ...
  bindGo(c);bindDashboardDrill(c,latest,previous,topBranch);bindUnifiedAnalysis(c);
}
