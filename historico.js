...
function renderYearMonthCard(matrix,month,maxPB){
  const period=matrix.byMonth.get(month);if(!period)return `<article class="hist-story-month pending"><div><strong>${MONTHS_H[month]}</strong><span>Cierre pendiente</span></div></article>`;
  const categoryRows=matrix.rows.filter(row=>row.category),pb=matrix.values.get(`ownerOrg:${month}`)||0,passRows=matrix.rows.filter(row=>row.id&&row.id.startsWith('pass:')),passes=passRows.reduce((sum,row)=>sum+(matrix.values.get(`${row.id}:${month}`)||0),0),bonus=(matrix.values.get(`bonus1:${month}`)||0)+(matrix.values.get(`bonus2:${month}`)||0),purged=matrix.values.get(`purged:${month}`)||0,incomes=num(period.summary&&period.summary.incomeCount),width=maxPB?Math.max(7,Math.round(pb/maxPB*100)):0;
  return `<article class="hist-story-month"><div class="hist-story-month-top"><div class="hist-story-month-name"><i></i><div><strong>${MONTHS_H[month]}</strong><span>Cierre guardado</span></div></div><button class="hist-story-pb" data-hist-annual="ownerOrg" data-period-id="${period.id}"><b>${fmt(pb)} PB</b><small>ORGANIZACIÓN</small></button></div><div class="hist-story-meter"><i style="width:${width}%"></i></div><div class="hist-story-cats">${categoryRows.map(row=>{const value=matrix.values.get(`${row.id}:${month}`)||0;return `<button data-hist-annual="${row.id}" data-period-id="${period.id}"><b>${esc(row.category)}</b><span>${fmt(value)}</span></button>`}).join('')||'<span class="hist-story-empty">Sin categorías</span>'}</div><div class="hist-story-moves"><span>＋ ${incomes} ingresos</span><span>↗ ${passes} pases</span><span>✦ ${bonus} Bonus</span>${purged?`<span>− ${purged} dep.</span>`:''}</div></article>`;
}
